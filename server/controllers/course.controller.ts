...
// get single course --- without purchasing  with cache maintaining
...
// get single course --- with purchasing with cache maintaining
...
export const getCourseByUser = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!courseExists) {
        return next(
          new ErrorHandler("You are not elligible to access this course", 400)
        );
      }
...
export const addQuestion = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      // the contentId is the (_id Field within courseData)
      if (!courseContent) {
        return next(new ErrorHandler("Invalid content id", 400));
      }
    ...
    try {
      ...
      if (req.user?._id === question.user._id) {
        ...
        await NotificationModel.create({
          ...
          message: `You have a new question reply in ${courseContent.title} course`,
        });
      } else {
        const data = {
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      await NotificationModel.create({
        user: req.user?._id,
        title: "New Review Recieved",
        message: `You have a new review in ${course?.name} course`,
      });
